...
import { GET_RECIPINS, GET_BOARDS, GET_BOARD_RECIPINS } from "./actionTypes"
...
export function getRecipins(recipeName) {
        ...
        const data = axios.get(`https://www.food2fork.com/api/search?key=${API_KEY}&q=${recipeName}&count=6`).then(res => res.data )
...
export function getUserBoards(userId) {
    ...
    const data = axios.get(`/api/boards/${userId}`)
        .then(res => res.data)
...
export function getBoardRecipins(boardId) {
    const data = axios.get(`/api/saved-recipins/${boardId}`)
        .then(res => res.data)
